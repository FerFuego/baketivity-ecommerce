...
?>
<div class="c-related-subscription">
	<div class="c-related-subscription__header">
		<h2 class="c-related-subscription__title"><?php esc_html_e( 'Related subscriptions', 'woocommerce-subscriptions' ); ?></h2>
	</div>
	
	<table class="shop_table shop_table_responsive my_account_orders woocommerce-orders-table woocommerce-MyAccount-subscriptions woocommerce-orders-table--subscriptions c-related-subscription__table">
		<thead>
			<tr>
				...
				<?php if (!wp_is_mobile()) : ?>
					<th class="subscription-next-payment order-date woocommerce-orders-table__header woocommerce-orders-table__header-order-date woocommerce-orders-table__header-subscription-next-payment"><span class="nobr"><?php echo esc_html_x( 'Processing next kit', 'table heading', 'woocommerce-subscriptions' ); ?></span></th>
				<?php endif; ?>
			...
			<?php foreach ( $subscriptions as $subscription_id => $subscription ) : ?>
				<tr class="order woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $subscription->get_status() ); ?>">
					...
					<td class="subscription-actions order-actions woocommerce-orders-table__cell woocommerce-orders-table__cell-subscription-actions woocommerce-orders-table__cell-order-actions">
						<a href="<?php echo esc_url( $subscription->get_view_order_url() ) ?>" class="woocommerce-button button c-related-subscription__view"><?php echo esc_html_x( 'View', 'view a subscription', 'woocommerce-subscriptions' ); ?></a>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>
